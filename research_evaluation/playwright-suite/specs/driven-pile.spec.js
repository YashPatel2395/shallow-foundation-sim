...
for (const runNumber of RUNS) {
  test(`Driven Pile Foundation module — full walkthrough (run ${runNumber})`, async ({ page }, testInfo) => {
    ...
    const result = await walkModule(page, {
      entryUrl: 'driven-pile.html',
      moduleName: 'Driven Pile Foundation',
      stagePrefix: 'DP',
    });

    if (result.stageResults.length > 0) {
      await page.screenshot({ path: path.join(shotDir, `DP-01_${browserEngine}_run${runNumber}_first_stage.png`), fullPage: true }).catch(() => {});
    }
    if (result.finalStateReached) {
      await page.screenshot({ path: path.join(shotDir, `DP-final_${browserEngine}_run${runNumber}_completed.png`), fullPage: true }).catch(() => {});
    } else {
      await page.screenshot({ path: path.join(shotDir, `DP-incomplete_${browserEngine}_run${runNumber}_failure.png`), fullPage: true }).catch(() => {});
    }

    appendModuleRunResult({
      module_run_id: `DP_${browserEngine}_run${runNumber}`,
      module: 'Driven Pile Foundation',
    ...
    result.stageResults.forEach((r, i) => {
      appendFunctionalResult({
        test_id: `DP-${String(i + 1).padStart(2, '0')}_${browserEngine}_run${runNumber}`,
        stage_id: `DP-${String(i + 1).padStart(2, '0')}`,
        module: 'Driven Pile Foundation',
