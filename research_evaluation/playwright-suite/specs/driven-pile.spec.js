...
for (const runNumber of RUNS) {
  test(`Driven Pile Foundation module — full walkthrough (run ${runNumber})`, async ({ page }, testInfo) => {
    ...
    const cpuLogPath = path.join(CPU_LOG_DIR, `DP_${browserEngine}_run${runNumber}.json`);
    ...
    const finalShotPath = path.join(shotDir, result.finalStateReached
      ? `DP-final_${browserEngine}_run${runNumber}_completed.png`
      : `DP-incomplete_${browserEngine}_run${runNumber}_failure.png`);
    ...
    result.stageResults.forEach((r) => {
      ...
      appendFunctionalResult({
        test_id: `DP-${n}_${browserEngine}_run${runNumber}`,
        stage_id: `DP-${n}`,
        module: 'Driven Pile Foundation',
